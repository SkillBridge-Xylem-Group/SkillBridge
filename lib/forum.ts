...
const DELETED_ANSWER_TEXT = "[deleted]";

/**
 * Delete a comment/reply. If it has no replies, the row is removed entirely.
 * If it has replies, the content is replaced with a "[deleted]" placeholder
 * so the thread structure (and other people's replies) stays intact.
 */
export async function deleteAnswer(
  supabase: SupabaseClient,
  params: { answerId: string; userId: string }
): Promise<{ data: { question_id: string } | null; error: { message: string } | null }> {
  const { data: existing } = await supabase
    .from("forum_answers")
  ...
  if (!existing) return { data: null, error: { message: "Comment not found." } };
  if (existing.user_id !== params.userId) {
    return { data: null, error: { message: "You can only delete your own comment." } };
  }

  const { count } = await supabase
    .from("forum_answers")
    .select("answer_id", { count: "exact", head: true })
    .eq("parent_answer_id", params.answerId);

  if ((count ?? 0) > 0) {
    const { error } = await supabase
      .from("forum_answers")
      .update({ content: DELETED_ANSWER_TEXT, image_url: null })
      .eq("answer_id", params.answerId)
      .eq("user_id", params.userId);
    if (error) return { data: null, error };
    return { data: { question_id: existing.question_id }, error: null };
  }

  await supabase.from("answer_votes").delete().eq("answer_id", params.answerId);
  const { error } = await supabase
    .from("forum_answers")
    .delete()
    .eq("answer_id", params.answerId)
...
/** Delete a question/post and its comments. Only the author can delete it. */
export async function deleteQuestion(
  supabase: SupabaseClient,
  params: { questionId: string; userId: string }
): Promise<{ data: { subforum_slug: string | null } | null; error: { message: string } | null }> {
  const { data: existing } = await supabase
    .from("forum_questions")
    .select("question_id, user_id, subforum_slug")
    .eq("question_id", params.questionId)
    .maybeSingle();

  if (!existing) return { data: null, error: { message: "Post not found." } };
  if (existing.user_id !== params.userId) {
    return { data: null, error: { message: "You can only delete your own post." } };
  }
  ...
  const answerIds = (answerRows ?? []).map((a) => a.answer_id as string);
  if (answerIds.length > 0) {
    await supabase.from("answer_votes").delete().in("answer_id", answerIds);
    await supabase.from("forum_answers").delete().eq("question_id", params.questionId);
  }
  ...
  if (error) return { data: null, error };
  return { data: { subforum_slug: existing.subforum_slug ?? null }, error: null };
}